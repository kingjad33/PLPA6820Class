...
graph1= ggplot(mycodata, aes(x=Treatment, y=DON, fill=Cultivar)) +
...
#Question 2
mycodata$Treatment2= factor(mycodata$Treatment, levels= c("NTC","Fg","Fg + 37","Fg + 40","Fg + 70"))
graph2= ggplot(mycodata, aes(x=Treatment2, y=DON, fill=Cultivar)) +
  geom_boxplot(position=position_dodge(0.5),outlier.color = "NA") +
  xlab("") + ylab("DON (ppm)") +
  geom_point(pch=21, alpha=0.6, position=position_jitterdodge(dodge.width=0.9)) +
  scale_fill_manual(values= colorchoice)+
